...
var substitutions = require('./substitutions.js');
...
function setStatements(input, output, callback) {
    ...
    var buildStatements = function(err, query) {
        output.statements =
            [{ 'statement': query, 'parameters': output.parameters }];
        callback(err);
    };

    if (output.query && output.substitutions) {
        substitutions.find(output.query, output.substitutions, buildStatements);
    } else if (output.query) {
        buildStatements(output.query);
    } else {
        callback();
    }
}
